  ...
  async start() {
    ...
    tasks.forEach(async ({ id, status }: { id: string; status: boolean }) => {
      const isImplementation =
        this.taskHandlerService.manager.isRegisteredTaskJob(id);

      // DB에 작업 등록이 되어있지만 작업 스크립트가 구현되지않은 경우.
      if (!isImplementation) {
        await this.taskHandlerService.notFoundTask(id);
      ...
      if (status) {
        // 작업 및 작업 리스너 시작
        await this.taskHandlerService.manager.startTaskJob(id);
        await this.taskHandlerService.manager.startTaskListenerJob(id);
        await this.taskHandlerService.handleInitialStart(id);
      } else {
        // 작업 리스너만 시작
        await this.taskHandlerService.manager.startTaskListenerJob(id);
      }
